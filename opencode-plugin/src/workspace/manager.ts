...
export class WorkspaceManager {
	...
	private async getGitInfo(
		workspacePath: string,
	): Promise<{ url: string; branch: string; commit: string } | null> {
		try {
			const urlResult = await this
				.shell`cd ${workspacePath} && git remote get-url origin`;
			const branchResult = await this
				.shell`cd ${workspacePath} && git branch --show-current`;
			const commitResult = await this
				.shell`cd ${workspacePath} && git rev-parse HEAD`;
	...
	async isGitDirty(workspacePath: string): Promise<boolean> {
		try {
			const result = await this
				.shell`cd ${workspacePath} && git status --porcelain`;
	...
	async cleanup(name: string): Promise<void> {
		...
		try {
			...
			console.error(
				`[station-plugin] Failed to cleanup workspace ${name}:`,
				err,
			);
		}
	}
