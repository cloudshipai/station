...
export class NATSClient {
	...
	async connect(): Promise<boolean> {
		try {
			...
			console.log(
				`[station-plugin] Connected to NATS: ${this.config.nats.url}`,
			);
			...
			console.warn(
				`[station-plugin] NATS connection failed, running in standalone mode: ${err}`,
			);
			return false;
		}
	}
	...
	async subscribe(
		...
		const sub = this.nc.subscribe(subject, {
			callback: async (err, msg) => {
				if (err) {
					console.error(
						`[station-plugin] Subscription error on ${subject}:`,
						err,
					);
					return;
				}
	...
	async publish(subject: string, data: string): Promise<void> {
		if (!this.nc) {
			console.warn(
				`[station-plugin] Cannot publish to ${subject}: not connected`,
			);
			return;
		}
	...
	async filePut(name: string, data: Uint8Array): Promise<boolean> {
		...
		try {
			...
			console.error(
				`[station-plugin] Object store put failed for ${name}:`,
				err,
			);
			return false;
		}
