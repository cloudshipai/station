...
export class TaskHandler {
	...
	async handle(taskData: string): Promise<void> {
		...
		try {
			...
			await publisher.stream(
				workspace.created ? "workspace_created" : "workspace_reused",
				{
			...
			await publisher.stream(
				session.created ? "session_created" : "session_reused",
				{
					session: {
						name: session.name,
			...
			const updatedSession = await this.sessionManager.getSessionInfo(
				task.session.name,
			);
			...
			await publisher.result({
				...
				session: {
					...
					opencodeID: session.opencodeID,
					messageCount:
						updatedSession?.messageCount || session.messageCount + 1,
				},
				workspace: {
	...
	private async executePrompt(
		...
		try {
			...
			if (isSessionNotFound) {
				console.error(
					`[station-plugin] Session ${sessionID} not found - possible race condition:`,
					promptError,
				);
				throw new Error(
					`Session ${sessionID} not found. OpenCode may not have persisted the session. Try again.`,
				);
			}
		...
		console.log(
			"[station-plugin] Prompt result keys:",
			Object.keys(result || {}),
		);
		...
		if (!result.data) {
			console.error(
				"[station-plugin] Prompt returned no data, full result:",
