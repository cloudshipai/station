...
func (m *mockToolDeps) GetMCPServerByName(ctx context.Context, name string, environmentID int64) (MCPServerInfo, error) {
	if m.serverErr != nil {
		return MCPServerInfo{}, m.serverErr
	}
	server, ok := m.servers[name]
	if !ok {
		return MCPServerInfo{}, errors.New("server not found")
	}
	return server, nil
}

func (m *mockToolDeps) CallTool(ctx context.Context, serverID int64, toolName string, input map[string]interface{}) (ToolCallResult, error) {
	if m.callErr != nil {
		return ToolCallResult{}, m.callErr
	}
	return m.toolResult, nil
}

func TestToolExecutor_SupportedTypes(t *testing.T) {
	...
	if types[0] != workflows.StepTypeTool {
		t.Errorf("expected StepTypeTool, got %v", types[0])
	}
}
...
func TestToolExecutor_Execute(t *testing.T) {
	tests := []struct {
		...
		wantErr     bool
		errType     error
		checkResult func(t *testing.T, result StepResult)
	}{
		{
			name: "successful tool call",
			deps: &mockToolDeps{
				servers: map[string]MCPServerInfo{
					"kubectl": {ID: 1, Name: "kubectl"},
				},
				toolResult: ToolCallResult{
			...
			checkResult: func(t *testing.T, result StepResult) {
				...
				if result.Output["server"] != "kubectl" {
					t.Errorf("expected server=kubectl, got %v", result.Output["server"])
				}
				if result.Output["tool"] != "list_pods" {
					t.Errorf("expected tool=list_pods, got %v", result.Output["tool"])
				}
				if result.NextStep != "next-step" {
					t.Errorf("expected next-step, got %s", result.NextStep)
				}
		...
		},
		{
			...
			runContext: map[string]interface{}{},
			wantErr:    true,
			errType:    ErrToolNameRequired,
		},
		{
			name: "server not found",
			deps: &mockToolDeps{
				servers:   map[string]MCPServerInfo{},
				serverErr: errors.New("server not found"),
			},
			step: workflows.ExecutionStep{
				ID:   "tool-server-not-found",
				Type: workflows.StepTypeTool,
				Raw: workflows.StateSpec{
					Type:   "tool",
					Server: "nonexistent",
		...
		},
		{
			...
			runContext: map[string]interface{}{},
			wantErr:    true,
			errType:    ErrToolCallFailed,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			executor := NewToolExecutor(tt.deps)
			result, err := executor.Execute(context.Background(), tt.step, tt.runContext)
