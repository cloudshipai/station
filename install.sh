...
print_next_steps() {
    ...
    echo -e "   ${YELLOW}${BINARY_NAME} init${NC}"
    echo ""
    echo "2. Start Station server:"
    echo -e "   ${YELLOW}${BINARY_NAME} serve${NC}"
    echo ""
    echo "3. Add Station to Claude Code:"
    echo -e "   ${YELLOW}claude mcp add station -- ${BINARY_NAME} mcp${NC}"
    echo ""
    echo "   Or add to your MCP client config (Claude Desktop, Cursor, etc.):"
    echo -e "   ${YELLOW}\"station\": { \"command\": \"${BINARY_NAME}\", \"args\": [\"mcp\"] }${NC}"
    echo ""
    echo "4. Open the Station UI:"
    echo -e "   ${YELLOW}http://localhost:8585${NC}"
    echo ""
