...
async function getCachedHighlighter(config = {}) {
  ...
  if (highlighterPromise === void 0) {
    highlighterPromise = (0, import_shikiji.getHighlighter)({
...
async function ensureLanguageIsLoaded(highlighter, language) {
  ...
  if (!loadedLanguages.has(language) && !(0, import_shikiji.isSpecialLang)(language)) {
    if (!Object.keys(import_shikiji.bundledLanguages).includes(language)) {
      language = "txt";
    }
...
// src/index.ts
var import_shikiji2 = require("shikiji");
async function loadShikiTheme(bundledThemeName) {
  const shikiTheme = (await import_shikiji2.bundledThemes[bundledThemeName]()).default;
