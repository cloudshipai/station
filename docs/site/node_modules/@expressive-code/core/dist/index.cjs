...
__export(src_exports, {
  AnnotationRenderPhaseOrder: () => AnnotationRenderPhaseOrder,
  ...
  PluginStyleSettings: () => PluginStyleSettings,
  PluginTexts: () => PluginTexts,
  addClassName: () => addClassName,
  annotationSortFn: () => annotationSortFn,
  changeAlphaToReachColorContrast: () => changeAlphaToReachColorContrast,
  changeLuminanceToReachColorContrast: () => changeLuminanceToReachColorContrast,
  chromaticRecolor: () => chromaticRecolor,
  codeLineClass: () => codeLineClass,
  cssVarReplacements: () => cssVarReplacements,
  darken: () => darken,
  ensureColorContrastOnBackground: () => ensureColorContrastOnBackground,
  formatTemplate: () => formatTemplate,
  getClassNames: () => getClassNames,
  getColorContrast: () => getColorContrast,
  getColorContrastOnBackground: () => getColorContrastOnBackground,
  getCssVarName: () => getCssVarName,
  getGroupIndicesFromRegExpMatch: () => getGroupIndicesFromRegExpMatch,
  getLuminance: () => getLuminance,
  getStableObjectHash: () => getStableObjectHash,
  kebabCase: () => kebabCase,
  lighten: () => lighten,
  mix: () => mix,
  multiplyAlpha: () => multiplyAlpha,
  onBackground: () => onBackground,
  replaceDelimitedValues: () => replaceDelimitedValues,
  resolveStyleVariants: () => resolveStyleVariants,
  runHooks: () => runHooks,
  setAlpha: () => setAlpha,
  ...
  validateExpressiveCodeAnnotation: () => validateExpressiveCodeAnnotation
});
...
var InlineStyleAnnotation = class extends ExpressiveCodeAnnotation {
  ...
  render({ nodesToTransform, styleVariants }) {
    ...
    return nodesToTransform.map((node) => {
      ...
      const transformedNode = (0, import_hastscript.h)("span", { style: buildStyleString(newStyles) }, node);
...
var import_hastscript3 = require("hastscript");
...
var import_hastscript2 = require("hastscript");
...
function renderLineToAst({ line, ...restContext }) {
  ...
  const partNodes = textParts.map((textPart) => (0, import_hastscript2.h)(null, [textPart]));
  ...
  annotations.forEach((annotation, annotationIndex) => {
    ...
    if (partIndices.length > 1) {
      ...
      if (!isPartiallyContainedInLaterAnnotations) {
        const mergedNode = (0, import_hastscript2.h)(
          null,
  ...
  if (partNodes.length === 0)
    partNodes.push((0, import_hastscript2.h)(null, "\n"));
  let lineNode = (0, import_hastscript2.h)(`div.${codeLineClass}`, partNodes);
  annotations.forEach((annotation) => {
    if (annotation.inlineRange)
      return;
...
function buildCodeBlockAstFromRenderedLines(renderedLines) {
  return (0, import_hastscript3.h)("pre", { tabindex: 0 }, (0, import_hastscript3.h)("code", renderedLines));
}
...
var import_hastscript4 = require("hastscript");
...
var preprocessor = (0, import_postcss.default)([
...
var processor = (0, import_postcss.default)([
...
async function scopeAndMinifyNestedCss(css) {
  ...
  const root = import_postcss.default.parse(`.${groupWrapperClassName}{${css}}`, postCssOptions);
...
async function renderGroup({
  ...
  const groupRenderData = {
    groupAst: (0, import_hastscript4.h)(
      null,
...
function addWrapperAroundGroupAst({ groupAst }) {
  return (0, import_hastscript4.h)(`${groupWrapperElement}.${groupWrapperClassName}`, groupAst);
}
...
function getColorContrast(color1, color2) {
  ...
  return (0, import_tinycolor.readability)(color, backgroundColor);
}
function getColorContrastOnBackground(input, background) {
  const color = toTinyColor(input);
  const backgroundColor = toTinyColor(background);
  return (0, import_tinycolor.readability)(color.onBackground(backgroundColor), backgroundColor);
}
  ...
  return withParsedColor(input, (color) => {
    return withParsedColor(
      ...
      (backgroundColor) => {
        ...
        let curContrast = (0, import_tinycolor.readability)(newColor.onBackground(backgroundColor), backgroundColor);
        if (curContrast < minContrast) {
          const contrastWithoutAlpha = (0, import_tinycolor.readability)(newColor, backgroundColor);
          if (contrastWithoutAlpha < minContrast) {
            ...
            curContrast = (0, import_tinycolor.readability)(newColor.onBackground(backgroundColor), backgroundColor);
          }
        }
...
function changeLuminanceToReachColorContrast(input1, input2, minContrast = 6) {
  ...
  const oldContrast = (0, import_tinycolor.readability)(color1, color2);
  ...
  const lightenedContrast = (0, import_tinycolor.readability)(lightenedColor, color2);
  const darkenedContrast = (0, import_tinycolor.readability)(darkenedColor, color2);
...
function changeAlphaToReachColorContrast(input, background, minContrast = 6, maxContrast = 22) {
  ...
  const curContrast = (0, import_tinycolor.readability)(colorOnBackground, backgroundColor);
  if (curContrast >= minContrast && curContrast <= maxContrast)
  ...
  const newAlpha = binarySearch({
    getValueFn: (alpha) => {
      ...
      const result = (0, import_tinycolor.readability)(onBg, backgroundColor);
      return result;
    },
...
function toTinyColor(input) {
  if (input instanceof import_tinycolor.TinyColor) {
    return new import_tinycolor.TinyColor(input.toHexShortString());
  }
  if (typeof input === "string") {
    const labColor = parseCssLabColor(input);
    if (labColor) {
      return new import_tinycolor.TinyColor(labToRgba(labColor));
    }
    const lchColor = parseCssLchColor(input);
    if (lchColor) {
      return new import_tinycolor.TinyColor(lchabToRgba(lchColor));
    }
    const oklchColor = parseCssOklchColor(input);
    if (oklchColor) {
      return new import_tinycolor.TinyColor(oklchToRgba(oklchColor));
    }
    return new import_tinycolor.TinyColor(input);
  }
  if (typeof input === "object" && "mode" in input) {
    if (input.mode === "hsl")
      return new import_tinycolor.TinyColor(hslToRgba(input));
    if (input.mode === "oklch")
      return new import_tinycolor.TinyColor(oklchToRgba(input));
  }
  return new import_tinycolor.TinyColor(input);
}
function toHexColor(input) {
  const color = input instanceof import_tinycolor.TinyColor ? input : toTinyColor(input);
