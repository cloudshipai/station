...
var require_parse_numeric_range = __commonJS({
  "../../../node_modules/.pnpm/parse-numeric-range@1.3.0/node_modules/parse-numeric-range/index.js"(exports, module2) {
    "use strict";
...
__export(src_exports, {
  pluginTextMarkers: () => pluginTextMarkers,
  pluginTextMarkersData: () => pluginTextMarkersData
});
module.exports = __toCommonJS(src_exports);
var import_core4 = require("@expressive-code/core");
...
var textMarkersStyleSettings = new import_core.PluginStyleSettings({
  defaultValues: {
    textMarkers: {
...
function resolveBg({ resolveSetting: r }, hue) {
  return (0, import_core.toHexColor)(`lch(${r("textMarkers.defaultLuminance")} ${r("textMarkers.defaultChroma")} ${r(hue)} / ${r("textMarkers.backgroundOpacity")})`);
}
function resolveBorder({ resolveSetting: r }, hue) {
  return (0, import_core.toHexColor)(`lch(${r("textMarkers.borderLuminance")} ${r("textMarkers.defaultChroma")} ${r(hue)} / ${r("textMarkers.borderOpacity")})`);
}
function resolveIndicator({ resolveSetting: r }, hue) {
  return (0, import_core.toHexColor)(`lch(${r("textMarkers.indicatorLuminance")} ${r("textMarkers.defaultChroma")} ${r(hue)} / ${r("textMarkers.indicatorOpacity")})`);
}

// src/inline-markers.ts
var import_core2 = require("@expressive-code/core");
function getInlineSearchTermMatches(lineText, blockData) {
  const markerMatches = [];
  ...
  blockData.regExpTerms.forEach(({ markerType, regExp }) => {
    ...
    for (const match of matches) {
      const rawGroupIndices = (0, import_core2.getGroupIndicesFromRegExpMatch)(match);
...
// src/annotations.ts
var import_core3 = require("@expressive-code/core");
var import_hastscript = require("hastscript");
var TextMarkerAnnotation = class extends import_core3.ExpressiveCodeAnnotation {
  markerType;
  backgroundColor;
  label;
  ...
  renderFullLineMarker({ nodesToTransform }) {
    return nodesToTransform.map((node) => {
      (0, import_core3.addClassName)(node, this.markerType);
      if (this.label) {
        (0, import_core3.setProperty)(node, "data-marker-label", this.label);
      }
      return node;
    });
  }
  renderInlineMarker({ nodesToTransform }) {
    return nodesToTransform.map((node, idx) => {
      const transformedNode = (0, import_hastscript.h)(this.markerType, node);
      if (nodesToTransform.length > 0 && idx > 0) {
        (0, import_core3.addClassName)(transformedNode, "open-start");
      }
      if (nodesToTransform.length > 0 && idx < nodesToTransform.length - 1) {
        (0, import_core3.addClassName)(transformedNode, "open-end");
      }
  ...
  return {
    ...
    hooks: {
      preprocessMetadata: ({ codeBlock, cssVar }) => {
        ...
        codeBlock.meta = (0, import_core4.replaceDelimitedValues)(
          codeBlock.meta,
          ({ fullMatch, key, value, valueStartDelimiter }) => {
            ...
            if (valueStartDelimiter === "{") {
              let label = void 0;
              value = value.replace(/^\s*?(["'])([^\1]+?)\1:\s*?/, (_match, _quote, labelValue) => {
                label = labelValue;
                return "";
              });
              const lineNumbers = (0, import_parse_numeric_range.default)(value);
      ...
      postprocessAnnotations: ({ codeBlock, styleVariants, config }) => {
        ...
        codeBlock.getLines().forEach((line) => {
          ...
          for (const annotation of annotations) {
            ...
            if (fullLineMarker) {
              if (MarkerTypeOrder.indexOf(annotation.markerType) < MarkerTypeOrder.indexOf(fullLineMarker.markerType))
                continue;
              if (import_core4.AnnotationRenderPhaseOrder.indexOf(annotation.renderPhase) < import_core4.AnnotationRenderPhaseOrder.indexOf(fullLineMarker.renderPhase))
                continue;
            }
          ...
          styleVariants.forEach((styleVariant, styleVariantIndex) => {
            ...
            const lineBgColor = (0, import_core4.onBackground)(fullLineMarkerBgColor, styleVariant.resolvedStyleSettings.get("codeBackground") || styleVariant.theme.bg);
            ...
            textColors.forEach((textColor) => {
              ...
              markers.forEach((marker) => {
                ...
                const combinedBgColor = (0, import_core4.onBackground)(markerBgColor, lineBgColor);
                const readableTextColor = (0, import_core4.ensureColorContrastOnBackground)(textFgColor, combinedBgColor, config.minSyntaxHighlightingColorContrast);
                if (readableTextColor.toLowerCase() === textFgColor.toLowerCase())
                  return;
                line.addAnnotation(
                  new import_core4.InlineStyleAnnotation({
                    styleVariantIndex,
                    inlineRange: {
...
var pluginTextMarkersData = new import_core4.AttachedPluginData(() => ({ plaintextTerms: [], regExpTerms: [] }));
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  pluginTextMarkers,
  pluginTextMarkersData
});
